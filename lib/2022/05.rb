...
input.take_while { |line| line[1].to_i != 1 }.map do |l|
  ...
  l.chars.each_slice(4) do |a|
    if a[1] != " " then CRATES[x] = (CRATES[x] ||= []) << a[1] end
    x += 1
  end
end
CRATES.freeze

def rearrange_crates_one_by_one(crates)
  MOVES.each do |m|
    x = 0
    while x < m[0]
...
def rearrange_crates_by_group(crates)
  MOVES.each do |m|
...
solve!(
  ...
  top_of_stacks(CRATES.map(&:dup), method(:rearrange_crates_one_by_one)),
)

solve!(
  "what crate ends up on top of each stack after the groups at a time rearrangement procedure completes:",
  top_of_stacks(CRATES.map(&:dup), method(:rearrange_crates_by_group)),
)
